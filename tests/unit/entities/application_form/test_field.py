...
_CHOICES = (FieldChoice(value="a", label="A"), FieldChoice(value="b", label="B"))
...
def test_blank_name_is_rejected(blank: str) -> None:
    ...
    with pytest.raises(InvalidApplicationFormDataError, match="Field name must not be empty"):
        Field(name=blank, label="Label", type=FieldType.STRING)


@pytest.mark.parametrize("blank", ["", "   ", "\t"])
def test_blank_label_is_rejected(blank: str) -> None:
    """Blank field label raises InvalidApplicationFormDataError."""
    with pytest.raises(InvalidApplicationFormDataError, match="Field label must not be empty"):
        Field(name="name", label=blank, type=FieldType.STRING)
...
def test_select_type_without_choices_is_rejected(ft: FieldType) -> None:
    ...
    with pytest.raises(InvalidApplicationFormDataError, match="must have at least one choice"):
        Field(name="pick", label="Pick", type=ft)
...
def test_select_type_with_empty_choices_is_rejected(ft: FieldType) -> None:
    ...
    with pytest.raises(InvalidApplicationFormDataError, match="must have at least one choice"):
        Field(name="pick", label="Pick", type=ft, choices=())
...
def test_scalar_type_with_choices_is_rejected(ft: FieldType) -> None:
    ...
    with pytest.raises(InvalidApplicationFormDataError, match="must not have choices"):
        Field(name="x", label="X", type=ft, choices=_CHOICES)
...
def test_duplicate_choice_values_are_rejected(ft: FieldType) -> None:
    ...
    dup_choices = (FieldChoice(value="x", label="First"), FieldChoice(value="x", label="Second"))

    with pytest.raises(InvalidApplicationFormDataError, match="duplicate choice values"):
        Field(name="pick", label="Pick", type=ft, choices=dup_choices)


@given(name=valid_text(), label=valid_text())
def test_valid_string_field_is_accepted(name: str, label: str) -> None:
    """Any non-blank name and label produce a valid STRING field."""
    field = Field(name=name, label=label, type=FieldType.STRING)

    assert field == Field(name=name, label=label, type=FieldType.STRING, required=True, choices=None)


@given(name=valid_text(), label=valid_text())
def test_valid_int_field_is_accepted(name: str, label: str) -> None:
    """Any non-blank name and label produce a valid INT field."""
    field = Field(name=name, label=label, type=FieldType.INT, required=False)
...
@given(name=valid_text(), label=valid_text())
def test_valid_select_field_is_accepted(name: str, label: str) -> None:
    """Any non-blank name and label produce a valid SELECT field."""
    field = Field(name=name, label=label, type=FieldType.SELECT, choices=_CHOICES)

    assert field.choices == _CHOICES


@given(name=valid_text(), label=valid_text())
def test_valid_multiselect_field_is_accepted(name: str, label: str) -> None:
    """Any non-blank name and label produce a valid MULTISELECT field."""
    field = Field(name=name, label=label, type=FieldType.MULTISELECT, choices=_CHOICES)
