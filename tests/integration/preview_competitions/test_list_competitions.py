...
from tests.integration.conftest import USER_ID, create_competition, create_competitions
...
async def test_preview_list_competitions_succeeds(
    ...
    with api_client.authenticate(auth_user_id=USER_ID):
        form1 = competition_form_factory.build()
        schedule1 = replace(
            form1.schedule,
            registration_start=now,
            registration_end=now + timedelta(days=5),
        )
        comp1 = await create_competition(
            form1.model_copy(update={"schedule": schedule1, "is_archived": False}),
            api_client
        )

        form2 = competition_form_factory.build()
        schedule2 = replace(
            form2.schedule,
            registration_start=now,
            registration_end=now + timedelta(days=5),
        )
        comp2 = await create_competition(
            form2.model_copy(update={"schedule": schedule2, "is_archived": False}),
            api_client
        )

        form3 = competition_form_factory.build()
        schedule3 = replace(
            form3.schedule,
            registration_start=now,
            registration_end=now + timedelta(days=5),
        )
        comp3 = await create_competition(
            form3.model_copy(update={"schedule": schedule3, "is_archived": False}),
            api_client
        )
...
async def test_preview_list_competitions_filters_archived(
    ...
    with api_client.authenticate(auth_user_id=USER_ID):
        ...
        active_competition = await create_competition(
            active_form.model_copy(update={"schedule": schedule, "is_archived": False}),
            api_client
        )

        archived_form = competition_form_factory.build()
        schedule = replace(
            archived_form.schedule,
            registration_start=now,
            registration_end=now + timedelta(days=5),
        )
        archived_competition = await create_competition(
            archived_form.model_copy(update={"schedule": schedule, "is_archived": False}),
            api_client
        )

        # Отмечаем как архивное после создания
...
async def test_preview_list_competitions_filters_by_registration(
    ...
    with api_client.authenticate(auth_user_id=USER_ID):
        ...
        active_form = active_form.model_copy(update={"schedule": active_schedule, "is_archived": False})
        ...
        expired_form = expired_form.model_copy(update={"schedule": expired_schedule, "is_archived": False})
...
async def test_preview_list_competitions_sorted_by_created_at_desc(
    ...
    with api_client.authenticate(auth_user_id=USER_ID):
        form1 = competition_form_factory.build()
        schedule1 = replace(form1.schedule, registration_end=now + timedelta(days=5))
        comp1 = await create_competition(form1.model_copy(update={"schedule": schedule1, "is_archived": False}), api_client)

        form2 = competition_form_factory.build()
        schedule2 = replace(form2.schedule, registration_end=now + timedelta(days=5))
        comp2 = await create_competition(form2.model_copy(update={"schedule": schedule2, "is_archived": False}), api_client)

        form3 = competition_form_factory.build()
        schedule3 = replace(form3.schedule, registration_end=now + timedelta(days=5))
        comp3 = await create_competition(form3.model_copy(update={"schedule": schedule3, "is_archived": False}), api_client)
