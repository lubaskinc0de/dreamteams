...
| `INVALID_JOB_STATUS` | Entity invariant | `JobStatus` value object constructed with inconsistent `reason` |
| `INVALID_JOB_STATUS_TRANSITION` | Entity invariant | `mark_success` / `mark_failed` called on a job that is not in `pending` |
