...
class ListApplicationsByCompetition:
    ...
    async def execute(
        ...
        logger.debug("Listing applications by competition", competition_id=competition_id, user_id=user.id, page=page)
        ...
        if not competition.can_read(user):
            ...
            raise AccessDeniedError(message="Only the organizer who owns this competition can list its applications")
